...
class CouchbasePyonDataStore(CouchbaseDataStore):
    """
    ...
    def __init__(self, datastore_name=None, profile=None, config=None, **kwargs):
        ...
        super(CouchbasePyonDataStore, self).__init__(datastore_name=datastore_name,
    ...
    def find_objects(self, subject, predicate=None, object_type=None, id_only=False, **kwargs):
        ...
        rows = ds.view(self._get_view_name("association", "by_sub"), start_key=key, end_key=endkey, stale="false", **view_args)

        '''
        import pprint
        print "\n\n view objects: ", self._get_view_name("association", "by_sub"), " key:", key, " endkey", endkey
    ...
    def find_subjects(self, subject_type=None, predicate=None, obj=None, id_only=False, **kwargs):
        ...
        endkey = self._get_endkey(key)
        #rows = view[key:endkey]
        rows = ds.view(self._get_view_name("association", "by_obj"), start_key=key, end_key=endkey, stale="false", **view_args)
    ...
    def find_associations(self, subject=None, predicate=None, obj=None, assoc_type=None, id_only=True, anyside=None, **kwargs):
        ...
        if subject and obj:
            ...
            if predicate:
                key.append(predicate)
            endkey = self._get_endkey(key)
            rows = ds.view(self._get_view_name("association", "by_match"), start_key=key, end_key=endkey, stale="false", **view_args)
        elif subject:
            key = [subject_id]
            if predicate:
                key.append(predicate)
            ...
            if predicate:
                ...
                endkey = self._get_endkey(key)
                rows = ds.view(self._get_view_name("association", "by_idpred"), start_key=key, end_key=endkey, stale="false", **view_args)
            ...
            key = [predicate]
            endkey = self._get_endkey(key)
            rows = ds.view(self._get_view_name("association", "by_pred"), start_key=key, end_key=endkey, stale="false", **view_args)
        else:
    ...
    def find_res_by_type(self, restype, lcstate=None, id_only=False, filter=None):
        ...
        if restype:
            ...
            endkey = self._get_endkey(key)
            ##rows = view[key:endkey]
            rows = ds.view(self._get_view_name("resource", "by_type"), include_docs=(not id_only), start_key=key, end_key=endkey, stale="false", **filter)
        else:
    ...
    def find_res_by_lcstate(self, lcstate, restype=None, id_only=False, filter=None):
        ...
        endkey = self._get_endkey(key)
        ##rows = view[key:endkey]
        rows = ds.view(self._get_view_name("resource", "by_lcstate"), include_docs=(not id_only),  start_key=key, end_key=endkey, stale="false", **filter)
    ...
    def find_res_by_name(self, name, restype=None, id_only=False, filter=None):
        ...
        endkey = self._get_endkey(key)
        #rows = view[key:endkey]
        rows = ds.view(self._get_view_name("resource", "by_name"), include_docs=(not id_only), start_key=key, end_key=endkey, stale="false", **filter)
    ...
    def find_res_by_keyword(self, keyword, restype=None, id_only=False, filter=None):
        ...
        endkey = self._get_endkey(key)
        ##rows = view[key:endkey]
        rows = ds.view(self._get_view_name("resource", "by_keyword"), include_docs=(not id_only), start_key=key, end_key=endkey, stale="false", **filter)
    ...
    def find_res_by_nested_type(self, nested_type, restype=None, id_only=False, filter=None):
        ...
        endkey = self._get_endkey(key)
        #rows = view[key:endkey]
        rows = ds.view(self._get_view_name("resource", "by_nestedtype"), include_docs=(not id_only), start_key=key, end_key=endkey, stale="false", **filter)
    ...
    def find_res_by_attribute(self, restype, attr_name, attr_value=None, id_only=False, filter=None):
        ...
        endkey = self._get_endkey(key)
        ##rows = view[key:endkey]
        rows = ds.view(self._get_view_name("resource", "by_attribute"), include_docs=(not id_only), start_key=key, end_key=endkey, stale="false", **filter)
    ...
    def find_res_by_alternative_id(self, alt_id=None, alt_id_ns=None, id_only=False, filter=None):
        ...
        endkey = self._get_endkey(key)
        ##rows = view[key:endkey]
        rows = ds.view(self._get_view_name("resource", "by_altid"), include_docs=(not id_only), start_key=key, end_key=endkey, stale="false", **filter)
