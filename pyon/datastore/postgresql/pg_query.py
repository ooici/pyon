...
class PostgresQueryBuilder(object):
    ...
    def _build_where(self, expr):
        ...
        elif op == DQ.XOP_ATTLIKE:
            attname, value = args
            return "json_string(doc,%s) LIKE %s" % (self._value(attname), self._value(value))
        elif op == DQ.XOP_ALLMATCH:
            value = args[0]
            return "json_allattr(doc) LIKE %s" % (self._value("%" + str(value) + "%"))
        elif op == DQ.GOP_OVERLAPS_BBOX:
            colname, x1, y1, x2, y2 = args
            return "%s && ST_MakeEnvelope(%s,%s,%s,%s,4326)" % (colname,
                    self._value(x1), self._value(y1), self._value(x2), self._value(y2))
        elif op == DQ.GOP_CONTAINS_BBOX:
            colname, x1, y1, x2, y2 = args
            return "%s ~ ST_MakeEnvelope(%s,%s,%s,%s,4326)" % (colname,
                    self._value(x1), self._value(y1), self._value(x2), self._value(y2))
        elif op == DQ.GOP_CONTAINEDBY_BBOX:
            colname, x1, y1, x2, y2 = args
            return "%s @ ST_MakeEnvelope(%s,%s,%s,%s,4326)" % (colname,
                    self._value(x1), self._value(y1), self._value(x2), self._value(y2))
        elif op == DQ.EXP_AND:
